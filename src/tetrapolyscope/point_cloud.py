...
from tetrapolyscope.core import str_to_datatype, str_to_vectortype, glm3, str_to_point_render_mode, point_render_mode_to_str, str_to_param_coords_type
from tetrapolyscope.structure import Structure
from tetrapolyscope.common import process_quantity_args, process_scalar_args, process_color_args, process_vector_args, process_parameterization_args, check_all_args_processed
...
class PointCloud(Structure):
    ...
    def add_color_quantity(self, name, values, **color_args):
        if len(values.shape) != 2 or values.shape[0] != self.n_points() or values.shape[1] != 3:
    ...
    def add_tetracolor_quantity(self, name, values, **tetracolor_args):
        if len(values.shape) != 2 or values.shape[0] != self.n_points() or values.shape[1] != 4:
            raise ValueError("'values' should be an Nx4 array")
    ...
    def add_vector_quantity(self, name, values, vectortype="standard", **vector_args):
        if len(values.shape) != 2 or values.shape[0] != self.n_points() or values.shape[1] not in [2, 3]:
    ...
    def add_parameterization_quantity(self, name, values, coords_type='unit', **parameterization_args):
        ...
        if len(values.shape) != 2 or values.shape[0] != self.n_points() or values.shape[1] != 2:

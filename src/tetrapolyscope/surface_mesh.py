...
class SurfaceMesh(Structure):
    ...
    # Tetracolor
    def add_tetracolor_quantity(self, name, values, defined_on='vertices', param_name=None, image_origin="upper_left", **tetracolor_args):
        if defined_on != 'texture' and (len(values.shape) != 2 or values.shape[1] != 4): raise ValueError("'values' should be an Nx4 array")

        if defined_on == 'vertices':
            if values.shape[0] != self.n_vertices(): raise ValueError("'values' should be a length n_vertices array")
            q = self.bound_instance.add_vertex_tetracolor_quantity(name, values)
        elif defined_on == 'faces':
            if values.shape[0] != self.n_faces(): raise ValueError("'values' should be a length n_faces array")
            q = self.bound_instance.add_face_tetracolor_quantity(name, values)
        elif defined_on == 'texture':
            raise ValueError("tetracolor texture not yet supported")
        else:
            raise ValueError("bad `defined_on` value {}, should be one of ['vertices', 'faces', 'texture']".format(defined_on))

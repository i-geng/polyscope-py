...
import tetratetrapolyscope.imgui as psim
...
def main():
    ...
    def callback():
        ...
        if is_test_clicking:
            ...
            if io.MouseClicked[0]:
                ...
                pick_result = tetrapolyscope.pick(screen_coords=screen_coords)
                print(pick_result)
                is_test_clicking = False
    ...
    tetrapolyscope.set_user_callback(callback)
    ...
    # tetrapolyscope.set_ground_plane_mode("shadow_only")
    tetrapolyscope.set_verbosity(101)
    ...
    if args.mesh:
        ...
        ps_mesh = tetrapolyscope.register_surface_mesh("test mesh", verts, faces, enabled=True)
        ...
        if corner_UVs is not None:
            ...
            ps_mesh.add_color_quantity("color_tex", color_tex, defined_on='texture',
                                       param_name="param", image_origin='upper_left')
            ps_mesh.add_scalar_quantity("scalar_tex", color_tex[:, :, 1], defined_on='texture', param_name="param")
            tetrapolyscope.add_color_image_quantity("my im", color_tex)
        ...
        tetrapolyscope.get_surface_mesh("test mesh").add_scalar_quantity(
            "Z", verts[:, 2])  # this time, get mesh by name
        tetrapolyscope.show()

        # Remove the whole mesh structure
        tetrapolyscope.remove_all_structures()

    # Examples with a point cloud
    if args.pointcloud:
        ps_points = tetrapolyscope.register_point_cloud("test points", verts, point_render_mode='sphere')
        ...
        tetrapolyscope.get_point_cloud("test points").add_scalar_quantity(
            "Z", verts[:, 2])  # this time, get structure by name
        # ps_points.set_point_radius_quantity("Y")
        tetrapolyscope.show()

        # Add some vectors
        vecs = np.random.rand(verts.shape[0], 3) - 0.5
        tetrapolyscope.get_point_cloud("test points").add_vector_quantity("vecs", vecs)

        # Add some colors
        colors = np.random.rand(verts.shape[0], 3)
        tetrapolyscope.get_point_cloud("test points").add_color_quantity("colors", colors)
    ...
    if args.volumemesh:
        ...
        ps_vol = tetrapolyscope.register_volume_mesh("test volume mesh", verts, mixed_cells=cells)
        ...
        # alternately:
        tetrapolyscope.register_volume_mesh("my mesh2", verts, mixed_cells=cells, enabled=False,
        ...
        # ps_vol = tetrapolyscope.register_volume_mesh("test volume mesh", verts, tets=cells)

        verts, tets, _ = igl.read_mesh("my_mesh.mesh")
        ps_vol = tetrapolyscope.register_volume_mesh("test volume mesh", verts, tets=tets)
    ...
    if args.volumegrid:
        ...
        ps_grid = tetrapolyscope.register_volume_grid("test volume grid", bound_min, bound_max, node_dims)
    ...
    # tetrapolyscope.clear_user_callback()


def implicit_ui():

    if (psim.Button("Render sphere SDF")):
        # tetrapolyscope.render_implicit_surface("sphere sdf", sphere_sdf, 'sphere_march', enabled=True)
        tetrapolyscope.render_implicit_surface_color("sphere sdf", sphere_sdf, color_func, 'sphere_march', enabled=True)

import tetrapolyscope.implot as psimplot
...
class TestSurfaceMesh(unittest.TestCase):
    ...
    def test_scalar(self):
        ...
        for on in ['vertices', 'faces', 'edges', 'halfedges', 'texture']:
            ...
            p.add_scalar_quantity("test_vals_with_range", vals, defined_on=on,
                                  param_name=param_name, vminmax=(-5., 5.), enabled=True)
            p.add_scalar_quantity("test_vals_with_datatype", vals, defined_on=on,
                                  param_name=param_name, enabled=True, datatype='symmetric')
            p.add_scalar_quantity("test_vals_with_cmap", vals, defined_on=on,
                                  param_name=param_name, enabled=True, cmap='blues')
    ...
    def test_distance(self):
        ...
        for on in ['vertices']:
            ...
            p.add_distance_quantity("test_vals_with_stripe_reltrue", vals, defined_on=on,
                                    enabled=True, stripe_size=0.01, stripe_size_relative=True)
            p.add_distance_quantity("test_vals_with_stripe_relfalse", vals, defined_on=on,
                                    enabled=True, stripe_size=0.01, stripe_size_relative=False)
    ...
    def test_tangent_vector(self):
        ...
        for on in ['vertices', 'faces']:
            ...
            p.add_tangent_vector_quantity("test_vals3", vals, basisX, basisY,
                                          defined_on=on, enabled=True, vectortype='ambient')
            ...
            p.add_tangent_vector_quantity("test_vals6", vals, basisX, basisY,
                                          defined_on=on, enabled=True, color=(0.2, 0.5, 0.5))
...
class TestVolumeGrid(unittest.TestCase):
    ...
    def test_scalar(self):
        ...
        for on in ['nodes', 'cells']:
            ...
            p.add_scalar_quantity("test_vals_with_datatype", vals, defined_on=on, enabled=True, datatype='symmetric')
            p.add_scalar_quantity("test_vals_with_cmap", vals, defined_on=on,
                                  enabled=True, cmap='blues', enable_gridcube_viz=False)
    ...
    def test_scalar_from_callable(self):
        ...
        for on in ['nodes', 'cells']:
            ...
            p.add_scalar_quantity_from_callable("test_vals_with_range", sphere_sdf,
                                                defined_on=on, vminmax=(-5., 5.), enabled=True)
            p.add_scalar_quantity_from_callable("test_vals_with_datatype", sphere_sdf,
                                                defined_on=on, enabled=True, datatype='symmetric')
            p.add_scalar_quantity_from_callable("test_vals_with_cmap", sphere_sdf,
                                                defined_on=on, enabled=True, cmap='blues', enable_gridcube_viz=False)
...
class TestCameraView(unittest.TestCase):
    ...
    def test_floating_scalar_images(self):
        ...
        cam.add_scalar_image_quantity("scalar_img2", np.zeros((dimY, dimX)), enabled=True, image_origin='lower_left',
                                      datatype='symmetric', vminmax=(-3., .3), cmap='reds', show_in_camera_billboard=True)
        cam.add_scalar_image_quantity("scalar_img3", np.zeros((dimY, dimX)), enabled=True,
                                      show_in_imgui_window=True, show_in_camera_billboard=False)
        cam.add_scalar_image_quantity("scalar_img4", np.zeros((dimY, dimX)), enabled=True,
                                      show_fullscreen=True, show_in_camera_billboard=False, transparency=0.5)

        # true floating adder
        ps.add_scalar_image_quantity("scalar_img2", np.zeros((dimY, dimX)), enabled=True,
                                     image_origin='lower_left', datatype='symmetric', vminmax=(-3., .3), cmap='reds')
    ...
    def test_floating_color_images(self):
        ...
        cam.add_color_image_quantity("color_img2", np.zeros((dimY, dimX, 3)), enabled=True,
                                     image_origin='lower_left', show_in_camera_billboard=True)
        cam.add_color_image_quantity("color_img3", np.zeros((dimY, dimX, 3)), enabled=True,
                                     show_in_imgui_window=True, show_in_camera_billboard=False)
        cam.add_color_image_quantity("color_img4", np.zeros((dimY, dimX, 3)), enabled=True,
                                     show_fullscreen=True, show_in_camera_billboard=False, transparency=0.5)

        # true floating adder
        ps.add_color_image_quantity("color_img2", np.zeros((dimY, dimX, 3)), enabled=True,
                                    image_origin='lower_left', show_in_camera_billboard=False)
    ...
    def test_floating_color_alpha_images(self):
        ...
        cam.add_color_alpha_image_quantity("color_alpha_img2", np.zeros(
            (dimY, dimX, 4)), enabled=True, image_origin='lower_left', show_in_camera_billboard=True)
        cam.add_color_alpha_image_quantity("color_alpha_img3", np.zeros(
            (dimY, dimX, 4)), enabled=True, show_in_imgui_window=True, show_in_camera_billboard=False, is_premultiplied=True)
        cam.add_color_alpha_image_quantity("color_alpha_img4", np.zeros(
            (dimY, dimX, 4)), enabled=True, show_fullscreen=True, show_in_camera_billboard=False)

        # true floating adder
        ps.add_color_alpha_image_quantity("color_alpha_img3", np.zeros(
            (dimY, dimX, 4)), enabled=True, show_in_imgui_window=True, show_in_camera_billboard=False)
    ...
    def test_floating_depth_render_images(self):
        ...
        cam.add_depth_render_image_quantity("render_img2", depths, normals, enabled=True,
                                            image_origin='lower_left', color=(0., 1., 0.), material='wax', transparency=0.7)

        # true floating adder
        ps.add_depth_render_image_quantity("render_img3", depths, normals, enabled=True, image_origin='lower_left', color=(
            0., 1., 0.), material='wax', transparency=0.7, allow_fullscreen_compositing=True)
    ...
    def test_floating_color_render_images(self):
        ...
        cam.add_color_render_image_quantity("render_img2", depths, normals, colors,
                                            enabled=True, image_origin='lower_left', material='wax', transparency=0.7)

        # true floating adder
        ps.add_color_render_image_quantity("render_img3", depths, normals, colors, enabled=True,
                                           image_origin='lower_left', material='wax', transparency=0.7, allow_fullscreen_compositing=True)
    ...
    def test_floating_scalar_render_images(self):
        ...
        cam.add_scalar_render_image_quantity("render_img2", depths, normals, scalars,
                                             enabled=True, image_origin='lower_left', material='wax', transparency=0.7)

        # true floating adder
        ps.add_scalar_render_image_quantity("render_img3", depths, normals, scalars, enabled=True,
                                            image_origin='lower_left', material='wax', transparency=0.7, allow_fullscreen_compositing=True)
    ...
    def test_floating_raw_color_render_images(self):
        ...
        cam.add_raw_color_render_image_quantity("render_img2", depths, colors,
                                                enabled=True, image_origin='lower_left', transparency=0.7)

        # true floating adder
        ps.add_raw_color_render_image_quantity("render_img3", depths, colors, enabled=True,
                                               image_origin='lower_left', transparency=0.7, allow_fullscreen_compositing=True)
    ...
    def test_floating_raw_color_alpha_render_images(self):
        ...
        cam.add_raw_color_alpha_render_image_quantity(
            "render_img2", depths, colors, enabled=True, image_origin='lower_left', transparency=0.7, is_premultiplied=True)

        # true floating adder
        ps.add_raw_color_alpha_render_image_quantity(
            "render_img3", depths, colors, enabled=True, image_origin='lower_left', transparency=0.7, allow_fullscreen_compositing=True)
    ...
    def test_floating_implicit_surface_render(self):
        ...
        # basic
        ps.render_implicit_surface("sphere sdf", sphere_sdf, 'fixed_step',
                                   subsample_factor=10, n_max_steps=10, enabled=True)
        ...
        cam = ps.register_camera_view("cam1", self.generate_parameters())
        ps.render_implicit_surface("sphere sdf", sphere_sdf, 'sphere_march', dim=(50, 75),
                                   n_max_steps=10, camera_view=cam, enabled=True)
    ...
    def test_floating_implicit_surface_color_render(self):
        ...
        # basic
        ps.render_implicit_surface_color("sphere sdf", sphere_sdf, color_func, 'fixed_step',
                                         subsample_factor=10, n_max_steps=10, enabled=True)
        ...
        # from this camera view
        cam = ps.register_camera_view("cam1", self.generate_parameters())
        ps.render_implicit_surface_color("sphere sdf", sphere_sdf, color_func, 'sphere_march', dim=(
            50, 75), n_max_steps=10, camera_view=cam, enabled=True)
    ...
    def test_floating_implicit_surface_scalar_render(self):
        ...
        # basic
        ps.render_implicit_surface_scalar("sphere sdf", sphere_sdf, scalar_func,
                                          'fixed_step', subsample_factor=10, n_max_steps=10, enabled=True)
        ...
        # from this camera view
        cam = ps.register_camera_view("cam1", self.generate_parameters())
        ps.render_implicit_surface_scalar("sphere sdf", sphere_sdf, scalar_func, 'sphere_march', dim=(
            50, 75), n_max_steps=10, camera_view=cam, enabled=True)
